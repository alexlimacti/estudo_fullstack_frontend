...
export class InterceptorService implements HttpInterceptor {
  ...
  handle303Error(error) {
    if (error.error.message === "invalidToken") {
      this.messageService.showError(
        "Vericação de registro",
        "Token Inválido, favor solicitar novo token"
      );
      return this.router.navigate(["resend-register-token"]);
    } else if (error.error.message === "expired") {
      this.messageService.showError(
        "Vericação de registro",
        "Token expirou, favor solicitar novo token"
      );
      return this.router.navigate(["resend-register-token"]);
    }
    return EmptyObservable.create();
  ...
  handle401Error(error) {
    if (error.error.error_description === "UserNotEnabled") {
      this.messageService.showError(
        "Usuário não está habilitado",
        "Favor habilitar o seu acesso atgravés do e-mail de verificação"
      );
      return this.router.navigate(["login"]);
    }
    return EmptyObservable.create();
